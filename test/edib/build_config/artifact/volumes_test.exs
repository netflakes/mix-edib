defmodule EDIBBuildConfigArtifactVolumesTest do
  use Pavlov.Case
  ...
  describe "EDIB.BuildConfig.Artifact.Volumes" do
    describe "from_cli_options/1" do
      context "happy path" do
        ...
        let :result,      do: EDIB.BuildConfig.Artifact.Volumes.from_cli_options(options)
        let :result_ok,   do: elem(result, 0)
        let :result_data, do: elem(result, 1)

        let :volume1,     do: EDIB.BuildConfig.Artifact.Volume.from_cli_option(vol1opt) |> elem(1)
        let :volume2,     do: EDIB.BuildConfig.Artifact.Volume.from_cli_option(vol2opt) |> elem(1)
      ...
      context "errors" do
        ...
        let :volume,      do: EDIB.BuildConfig.Artifact.Volume.from_cli_option(vol_opt)
    ...
    describe "to_docker_options/1" do
      ...
      let :volume1, do: EDIB.BuildConfig.Artifact.Volume.from_cli_option(vol1opt) |> elem(1)
      let :volume2, do: EDIB.BuildConfig.Artifact.Volume.from_cli_option(vol2opt) |> elem(1)
      let :result,  do: EDIB.BuildConfig.Artifact.Volumes.to_docker_options([volume1, volume2])
